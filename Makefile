BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Meine_Tibetreise_Band_Zwei"
TITLE_NAME = "Meine Tibetreise Band Zwei"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Meine_Tibetreise_II_*.jpg .
	cp  $(IMAGES_FOLDER)/NordChina_Ostibet.jpg .	
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Meine_Tibetreise_II_*.jpg
	rm NordChina_Ostibet.jpg
	rm cover.jpg 
	rm logo.jpg
